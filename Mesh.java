...
public class Mesh {
        ...
        while (sc.hasNext()) {
            ...
            if (line.charAt(0) == 'f') {
                ...
                triangles.add(new Triangle(vertices.get(verticies[0]), vertices.get(verticies[1]),
                        vertices.get(verticies[2]), location,false));
            }
        }
    }
    ...
    public Mesh(float[][][] vertices, Vec3D location) {
        for (int i = 0; i < vertices.length; i++) {
            triangles.add(new Triangle(
                    ...
                    new Vec3D(vertices[i][2][0], vertices[i][2][1], vertices[i][2][2]), location,false));
        }
    }
    ...
    void getTriangleAt(int scale, float xoffset, float yoffset, float yaw, float pitch, Vec3D camera, float fov, Graphics g) {
        ArrayList<Triangle> trianglesToDraw = new ArrayList<Triangle>();
        for (Triangle t : triangles) {
            // translate position before drawing
            Vec3D translatedLocation = Utilities.vecSub(t.getLocation(), camera);
